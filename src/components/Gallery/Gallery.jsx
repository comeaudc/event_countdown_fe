...
export default function Gallery() {
  ...
  const fetchMedia = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `https://event-countdown-be.onrender.com/api/photos?page=${page}&limit=20`,
  ...
  const saveCaption = async (id, caption) => {
    ...
    try {
      await axios.put(
        `https://event-countdown-be.onrender.com/api/photos/${id}`,
        { caption },
        { headers: { authorization: token } },
      );
      setMediaItems((prev) =>
  ...
  const nextModal = () =>
    setModalIndex((i) => (i < mediaItems.length - 1 ? i + 1 : i));
                ...
                {mediaType.startsWith("video") ? (
                  <div className="video-wrapper" onClick={() => openModal(i)}>
                    <video
                      ...
                      preload="metadata"
                      onLoadedMetadata={(e) =>
                        setSpans(e.target.closest(".gallery-card"))
                      }
                    />
          ...
          >
            <div className="modal-slide">
              {mediaItems[modalIndex].type.startsWith("video") ? (
                <video
                  src={mediaItems[modalIndex].imageUrl}
                  controls
                  autoPlay
              ...
              <div className="modal-caption">
                {mediaItems[modalIndex].caption || "No caption"}
              </div>
            </div>
            <button className="modal-close" onClick={closeModal}>
              ✕
            </button>
            <button className="modal-prev" onClick={prevModal}>
              ‹
            </button>
            <button className="modal-next" onClick={nextModal}>
              ›
            </button>
          </div>
        </div>
